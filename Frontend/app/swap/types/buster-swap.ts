...
export const BUSTER_TOKENS: Record<string, Token> = {
  ETH: {
    name: "Ethereum",
...
> {
  ...
  const onchainTokens = ["ETH", "USDC", "BSTR"];
...
export async function updateTokenData(): Promise<void> {
  try {
    ...
    Object.keys(BUSTER_TOKENS).forEach((tokenKey) => {
      if (dynamicTokens[tokenKey]) {
        const newToken = dynamicTokens[tokenKey];
        if (newToken.image) {
          BUSTER_TOKENS[tokenKey].image = newToken.image;
        }
        if (newToken.decimals) {
          BUSTER_TOKENS[tokenKey].decimals = newToken.decimals;
        }
        if (newToken.name) {
          BUSTER_TOKENS[tokenKey].name = newToken.name;
        }
      }
    });
